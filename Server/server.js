...
app.post('/api/upload', upload.single('image'), (req, res) => {
  ...
  const imageUrl = `http://192.168.0.41:${port}/uploads/${req.file.filename}`;
  res.json({ imageUrl });
});
...
app.put("/api/updatePost/:id", upload.single('image'), (req, res) => {
  ...
  const imageUrl = req.file ? `http://192.168.0.41:${port}/uploads/${req.file.filename}` : req.body.imageUrl;
