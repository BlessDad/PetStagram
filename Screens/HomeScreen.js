...
export default function HomeScreen() {
  ...
  const fetchPosts = async () => {
    try {
      const response = await axios.get('http://172.31.14.213:8080/api/getPost');
  ...
  const fetchComments = async (postId) => {
    try {
      const response = await axios.get(`http://52.78.86.212:8080/api/getComments/${postId}`);
  ...
  const handleCommentSubmit = async (postId) => {
    if (commentText.trim() !== '') {
      try {
        await axios.post('http://52.78.86.212:8080/api/addComment', {
          postId,
          comment: commentText,
        });
        setCommentText('');
  ...
  const handleDeleteComment = async (commentId, postId) => {
    try {
      await axios.delete(`http://52.78.86.212:8080/api/deleteComment/${commentId}`);
      await fetchComments(postId);
    } catch (error) {
